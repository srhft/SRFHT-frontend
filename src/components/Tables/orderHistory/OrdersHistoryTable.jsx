...
import { OrderLogsFetch_api } from '../../../API/api.js';
import { removeTrade } from '../../../redux/TradeBookSlice.js';
...
function OrdersHistoryTable() {
    ...
    useEffect(() => {
        (async() => {
            try {
                const {data} = await OrderLogsFetch_api();
    ...
    useEffect(() => {
        ...
        socket.connection.onmessage = (e) => {
            ...
            if (newData.event === 'manualtrade') gridApi.current.api.applyTransactionAsync({remove: [newData]});
            if(newData.event==='orderlogs'){         
                setColDef(gridApi, newData.result)
                gridApi.current.api.applyTransactionAsync({
                    add: [newData.result],
                    addIndex: 0,
                });
                dispatch(removeTrade(newData.orderno))
            }
        }
